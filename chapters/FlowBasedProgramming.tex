...
\par This program loops over the arguments the program is executed with, and prints them as strings to \codeword{stdout}. The arguments are never even written to any dynamic or static memory within the program. Flow-based programs with no nodes have no real effect on any arguments or data they consume. Now, with a single node, the flow looks similar:
